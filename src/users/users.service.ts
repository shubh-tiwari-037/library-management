...
export class UsersService {
  ...
  async validateCredentials(
    ...
    if (userMeta.passwordHash === passwordHash) {
      return {
        id: user.id,
        type: user.role === Role.Librarian ? UserType.Librarian : UserType.User,
      };
    }
  ...
  async create(data: {
    ...
    const user = await this.prisma.$transaction(async (task) => {
      const createUser = await task.user.create({
        data: {
          firstname: data.firstname,
          lastname: data.lastname,
          email: data.email.toLowerCase(),
  ...
  async setRole(userId: number, role: Role): Promise<User> {
    // await this.cacheManager.del(
    //   getAccessGuardCacheKey({ id: userId, role: Role.USER }),
    // );
    return await this.prisma.user.update({
      data: { role },
      where: {
